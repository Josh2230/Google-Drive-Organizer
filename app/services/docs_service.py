...
def list_drive_files(service) -> FileListResponse:
    """
...
def create_folder(year, service):
    ...
    folder = service.files().create(body=file_metadata, fields="id, name, createdTime").execute()
...
def move_files(file, parent, service):
    """
...
def find_folder(service, new_folder_name):
    """
...
def create_folders_by_date():
    ...
    for year, docs in grouped_by_years.items():
        ...
        folder_obj = find_folder(service, str(year))
        if not folder_obj:
            new_folder = create_folder(year, service)
        ...
        for doc in docs:
            try:
                move_files(doc, folder_obj, service)
